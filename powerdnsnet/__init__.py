...
class PowerdnsNet(object):
    """API client implementation of powerdns.net DNS Hosting"""
    ...
    def add_native_domain(self,zonename):
        ...
        result = self._soap_request('addNativeDomain',dict(domainName=zonename))
        self._clear_cache() # invalidate zone map. In the future we might check for existing map and add the new zone
    ...
    def add_record_to_zone(self,zone,name,content,rtype='A',ttl=3600,prio=0):
        ...
        zoneid=self._expect_zone_id(zone)
        args={
            'zoneId':zoneid,
            'Name':name,
            'Type':rtype,
        ...
        result=self._soap_request('addRecordToZone',args)
        record=self._node_to_record(result)
        return record

    def delete_all_records_for_domain(self,zone):
        """Clear all records in a domain (except autogenerated SOA and NS)"""
        zoneid=self._expect_zone_id(zone)
        result=self._soap_request('deleteAllRecordsForDomain',dict(zoneId=zoneid))

    def delete_record(self,record):
        """Delete a record (by id)"""
        recordid=self._expect_record_id(record)
        result=self._soap_request('deleteRecordById',dict(recordId=recordid))

    def delete_zone(self,zone):
        """Delete a zone (by name or id)"""
        zoneid=self._expect_zone_id(zone)
        result=self._soap_request('deleteZoneById',dict(zoneId=zoneid))
    ...
    def update_record(self,recordobject):
        ...
        result=self._soap_request('updateRecord',args)
    ...
    def list_records(self,zone,rtype=None):
        ...
        if rtype!=None: # TODO: doesn't work yet... argument order doesn't seem to be the prob. what else?
            recordlist = self._soap_request("listRecordsByType",dict(zoneId=zoneid,Type=rtype.upper()))
        else:
            recordlist = self._soap_request("listRecords",dict(zoneId=zoneid))
    ...
    def list_zones(self,updatecache=True):
        ...
        zoneids={}
        zonelist=self._soap_request("listZones")
    ...
    def renew_zone(self,zone):
        """Extend the lifetime of a zone for one year"""
        zoneid=self._expect_zone_id(zone)
        result=self._soap_request('renewZone',dict(zoneId=zoneid))

    def _clear_cache(self):
        """forget the zonename to id mapping"""
        self.cache_zonenames=None

    def _expect_zone_id(self,value):
        """makes sure 'value' is returned as a zone id.
        Input can be a zone id already, a zone name or a zone object"""
        if type(value)==int:
            return value
    ...
    def _expect_record_id(self,value):
        """makes sure 'value' is returned as a record id.
        Input can be a record id already or a record object"""
        if type(value)==int:
            return value
    ...
    def zoneid_by_name(self,zonename):
        """Returns the zoneid from the zone specified by name"""
...
<soap12:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap12="http://www.w3.org/2003/05/soap-envelope">
  <soap12:Body>"""
    ...
    def _soap_request(self,method,arguments=None):
